...
plugin for [RoboLens](https://github.com/robocurve/robolens) — the "Inspect AI
for robotics".

RoboLens factors a robotics eval into two swappable inputs: a `Policy` (the VLA
...
pip install robolens robolens-isaacsim
```

## Use it

The embodiment is discovered through the `robolens.embodiments` entry point, so
...
```python
from robolens_isaacsim import IsaacSimEmbodiment
...
RoboLens fail-fast-checks the `(policy, embodiment)` pair before any rollout. To
...
| `image_keys` / `state_keys` | map RoboLens keys → your task's raw dict keys |
...
- **Stream frames to disk for long episodes.** RoboLens keeps each step's
