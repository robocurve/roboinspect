...
This is the "body + world" half of a RoboLens eval, backed by a real physics
simulation. It conforms to :class:`robolens.Embodiment` (the runtime-checkable
protocol), so once installed it can be paired with any compatible
:class:`robolens.Policy` and run through ``robolens.eval``.

Design mirrors RoboLens's own ``RerunSink``: the heavy, GPU-bound, non-PyPI
...
on any machine (so ``robolens list embodiments`` and the fail-fast compatibility
...
from robolens.spaces import CANONICAL_STATE_UNITS

if TYPE_CHECKING:
    from robolens import Action
...
# GPU allows; RoboLens owns wall-clock pacing).
...
def _missing_isaac(exc: ImportError) -> RuntimeError:
    return RuntimeError(
        ...
        f"({exc}). The robolens-isaacsim adapter needs a working Isaac Lab "
...
def _default_state_fields(num_arm_joints: int) -> tuple[StateField, ...]:
    """Canonical proprioception for a Franka-like arm (keys/units from RoboLens)."""
...
class IsaacSimEmbodiment:
    """Wrap an Isaac Lab gym task as a RoboLens :class:`~robolens.Embodiment`.
    ...
    cameras:
        ...
        tuples. Match the names your policy requires (RoboLens remaps if needed).
    headless / device:
    ...
    def __init__(
        ...
        *,
        ...
        # Map RoboLens obs keys -> the task's raw dict keys. Identity by default.
    ...
    # ------------------------------------------------------------------ #
    # Translation: Isaac Lab dict/tensors <-> RoboLens types
    # ------------------------------------------------------------------ #
